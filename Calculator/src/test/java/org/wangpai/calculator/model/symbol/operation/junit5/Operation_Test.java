...
class Operation_Test {
    ...
    void add_Fraction()
        ...
        assertEquals(new Fraction(firstInt + secondInt),
                Operation.add(new Fraction(firstInt), new Fraction(secondInt)));
    }
    ...
    @Test
    void callBestOperation_Fraction() throws CalculatorException {
        assertEquals(new Fraction(firstInt + secondInt),
    ...
    void preliminaryCheck_normal() throws SyntaxException {
        assertDoesNotThrow(() -> preliminaryCheck_native(new Fraction(), new Fraction()));
    }

    @Test
    void preliminaryCheck_exception() {
        Throwable throwable = assertThrows(SyntaxException.class,
                () -> preliminaryCheck_native(new Fraction(), null, null));
    ...
    void sortByInheritance_normal() throws CalculatorException {
        // 方法调用时，发生了对形参的交换
        Object[] sortResult = sortByInheritance_native(new Fraction(), new RationalNumber());
        assertEquals(RationalNumber.class, sortResult[0].getClass());
        assertEquals(Fraction.class, sortResult[1].getClass());
        assertEquals(true, sortResult[2]);

        // 方法调用时，没有发生了对形参的交换
        sortResult = sortByInheritance_native(new RationalNumber(), new Fraction());
        assertEquals(RationalNumber.class, sortResult[0].getClass());
        assertEquals(Fraction.class, sortResult[1].getClass());
